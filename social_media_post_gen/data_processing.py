...
# Reads in data from excel file [cite: 1]
def load_data(file_path):
    return pd.read_excel(file_path)

# Scrapes article text from given URL [cite: 1]
def scrape_article(url):
    with sync_playwright() as p:
        ...
        try:
            ...
            # Specific site logic: check for buttons that reveal hidden content [cite: 1]
            try:
                # Generalized button query to find toggleable insights sections
            ...
            # Check URL for valid article [cite: 1]
            if "/articles/" not in page.url:
                browser.close()
                return None

            # Sections of the article to query [cite: 1]
            containers = page.query_selector_all(
                'div[class*="article"], div.cmp-text, article'
            )

            # Scrape paragraphs from main containers [cite: 1]
            text_blocks = []
            for container in containers:
                ...
                for p_tag in paragraphs:
                    ...
                    if not text:
                        continue
                    # Stop reading current paragraph if disclosures found [cite: 1]
                    if "Read important disclosures" in text:
                        break
                    text_blocks.append(text)

                # Stop reading completely if disclosures found [cite: 1]
                if any("Read important disclosures" in t for t in text_blocks):
                    break
            
            # Join and return scraped text [cite: 1]
            full_text = "\n\n".join(text_blocks)
            browser.close()
            return full_text
...
# Cleans scraped text [cite: 1]
def clean_text(text):
    # Replace non-breaking spaces with normal spaces [cite: 1]
    text = text.replace('\u00A0', ' ').replace("&nbsp;", " ").replace("\xa0", " ")

    # Remove unwanted sections such as footnotes, disclaimers, etc. [cite: 1]
    end_markers = [
    ...
    # Remove leading/trailing whitespace for each paragraph [cite: 1]
    paragraphs = [p.strip() for p in text.split('\n') if p.strip()]

    # Join paragraphs all in one line [cite: 1]
    text = " ".join(paragraphs)

    # Collapse multiple spaces/tabs into a single space [cite: 1]
    text = re.sub(r'[ \t]+', ' ', text).strip()

    return text

# Separates text into chunks for processing [cite: 1]
...
# Instantiate summarization pipeline [cite: 1]
summarizer = pipeline("summarization", model="facebook/bart-large-cnn", device=device)

# Summarizes text [cite: 1]
def summarize(batch):
    summaries = []
...
def main(path):
    ...
    # Group by unique URLs [cite: 1]
    unique_urls = train_data['URL'].dropna().unique()

    # Scrape articles [cite: 1]
    url_to_text = {}
